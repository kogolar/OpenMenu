...
final class ControlItem {
    ...
    private func configureCancellables() {
        ...
        if let appState {
            appState.settingsManager.generalSettingsManager.$showOpenMenuIcon
                .receive(on: DispatchQueue.main)
                .sink { [weak self] showOpenMenuIcon in
                    guard
                        let self,
                        !isSectionDivider
                    else {
            ...
            appState.settingsManager.generalSettingsManager.$openMenuIcon
                .receive(on: DispatchQueue.main)
                .sink { [weak self] _ in
                    guard let self else {
                        return
            ...
            appState.settingsManager.generalSettingsManager.$customOpenMenuIconIsTemplate
                .receive(on: DispatchQueue.main)
                .sink { [weak self] _ in
                    guard let self else {
                        return
            ...
            appState.settingsManager.generalSettingsManager.$useOpenMenuBar
                .receive(on: DispatchQueue.main)
                .sink { [weak self] useOpenMenuBar in
                    guard
                        let self,
                        let button = statusItem.button
                    else {
    ...
    private func updateStatusItem(with state: HidingState) {
        ...
        case .visible:
            ...
            let icon = appState.settingsManager.generalSettingsManager.openMenuIcon
    ...
    private func createMenu(with appState: AppState) -> NSMenu {
        ...
        let menu = NSMenu(title: "OpenMenu")

        let settingsItem = NSMenuItem(
            title: "OpenMenu Settings…",
