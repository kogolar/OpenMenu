...
final class MenuBarItemManager: ObservableObject {
    ...
    struct ItemCache: Hashable {
        ...
        /// The cached menu bar items managed by OpenMenu.
        ...
        /// Returns the cached menu bar items managed by OpenMenu for the given section.
        func managedItems(for section: MenuBarSection.Name) -> [MenuBarItem] {
            self[section].filter { item in
                ...
                if item.owningApplication == .current {
                    // OpenMenu icon is the only item owned by OpenMenu that should be included.
                    guard item.title == ControlItem.Identifier.openMenuIcon.rawValue else {
                        return false
                    }
                }
