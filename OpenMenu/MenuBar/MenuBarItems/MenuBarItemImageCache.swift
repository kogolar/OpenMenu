...
final class MenuBarItemImageCache: ObservableObject {
    ...
    func updateCache(sections: [MenuBarSection.Name]) async {
        ...
        let isOpenMenuBarPresented = await appState.navigationState.isOpenMenuBarPresented
        let isSearchPresented = await appState.navigationState.isSearchPresented

        if !isOpenMenuBarPresented && !isSearchPresented {
            guard await appState.navigationState.isAppFrontmost else {
                logSkippingCache(reason: "OpenMenu Bar not visible, app not frontmost")
                return
            }
            guard await appState.navigationState.isSettingsPresented else {
                logSkippingCache(reason: "OpenMenu Bar not visible, Settings not visible")
                return
            }
            guard case .menuBarLayout = await appState.navigationState.settingsNavigationIdentifier else {
                logSkippingCache(reason: "OpenMenu Bar not visible, Settings visible but not on Menu Bar Layout")
                return
            }
        }
    ...
    func updateCache() async {
        ...
        if isSettingsPresented || isSearchPresented {
            ...
            isOpenMenuBarPresented,
            let section = await appState.menuBarManager.openMenuBarPanel.currentSection
        {
