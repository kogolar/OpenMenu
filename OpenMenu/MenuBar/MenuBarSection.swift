...
final class MenuBarSection {
    ...
    /// A Boolean value that indicates whether the OpenMenu Bar should be used.
    private var useOpenMenuBar: Bool {
        appState?.settingsManager.generalSettingsManager.useOpenMenuBar ?? false
    }

    /// A weak reference to the menu bar manager's OpenMenu Bar panel.
    private weak var openMenuBarPanel: OpenMenuBarPanel? {
        appState?.menuBarManager.openMenuBarPanel
    }

    /// The best screen to show the OpenMenu Bar on.
    private weak var screenForOpenMenuBar: NSScreen? {
    ...
    var isHidden: Bool {
        if useOpenMenuBar {
            ...
            switch name {
            case .visible, .hidden:
                return openMenuBarPanel?.currentSection != .hidden
            case .alwaysHidden:
                return openMenuBarPanel?.currentSection != .alwaysHidden
            }
        }
        switch name {
        case .visible, .hidden:
            if openMenuBarPanel?.currentSection == .hidden {
                return false
            }
            return controlItem.state == .hideItems
        case .alwaysHidden:
            if openMenuBarPanel?.currentSection == .alwaysHidden {
                return false
            }
            return controlItem.state == .hideItems
        }
    ...
    convenience init(name: Name, appState: AppState) {
        ...
        case .visible:
            ControlItem(identifier: .openMenuIcon, appState: appState)
        case .hidden:
    ...
    func show() {
        ...
        switch name {
        case .visible where useOpenMenuBar, .hidden where useOpenMenuBar:
            Task {
                if let screenForOpenMenuBar {
                    await openMenuBarPanel?.show(section: .hidden, on: screenForOpenMenuBar)
                }
                for section in appState.menuBarManager.sections {
                    section.controlItem.state = .hideItems
                }
            }
        case .alwaysHidden where useOpenMenuBar:
            Task {
                if let screenForOpenMenuBar {
                    await openMenuBarPanel?.show(section: .alwaysHidden, on: screenForOpenMenuBar)
                }
                for section in appState.menuBarManager.sections {
                    section.controlItem.state = .hideItems
                }
