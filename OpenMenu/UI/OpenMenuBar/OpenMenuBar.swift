...
// MARK: - OpenMenuBarPanel

final class OpenMenuBarPanel: NSPanel {
    private weak var appState: AppState?

    private(set) var currentSection: MenuBarSection.Name?

    private lazy var colorManager = OpenMenuBarColorManager(openMenuBarPanel: self)
            ...
            window.publisher(for: \.frame)
                ...
                .sink { [weak self, weak window] _ in
                    guard
                        let self,
                        let appState,
                        // Only continue if the menu bar is automatically hidden, as OpenMenu
    ...
    private func updateOrigin(for screen: NSScreen) {
        ...
        func getOrigin(for openMenuBarLocation: OpenMenuBarLocation) -> CGPoint {
        ...
        setFrameOrigin(getOrigin(for: appState.settingsManager.generalSettingsManager.openMenuBarLocation))
    }
    ...
    func show(section: MenuBarSection.Name, on screen: NSScreen) async {
        ...
        appState.navigationState.isOpenMenuBarPresented = true
        ...
        contentView = OpenMenuBarHostingView(appState: appState, colorManager: colorManager, screen: screen, section: section) { [weak self] in
            self?.close()
        }
    ...
    override func close() {
        ...
        contentView = nil
        currentSection = nil
        appState?.navigationState.isOpenMenuBarPresented = false
    }
}

// MARK: - OpenMenuBarHostingView

private final class OpenMenuBarHostingView: NSHostingView<AnyView> {
    ...
    init(
        appState: AppState,
        colorManager: OpenMenuBarColorManager,
        screen: NSScreen,
        section: MenuBarSection.Name,
        closePanel: @escaping () -> Void
    ) {
        super.init(
            rootView: OpenMenuBarContentView(screen: screen, section: section, closePanel: closePanel)
...
// MARK: - OpenMenuBarContentView

private struct OpenMenuBarContentView: View {
    @EnvironmentObject var appState: AppState
    @EnvironmentObject var colorManager: OpenMenuBarColorManager
    ...
    private var content: some View {
        if !ScreenCapture.cachedCheckPermissions() {
            HStack {
                Text("The OpenMenu Bar requires screen recording permissions.")
            ...
            Text("OpenMenu cannot display menu bar items for automatically hidden menu bars")
                .padding(.horizontal, 10)
            ...
            ScrollView(.horizontal) {
                HStack(spacing: 0) {
                    ForEach(items, id: \.windowID) { item in
                        OpenMenuBarItemView(item: item, closePanel: closePanel)
                    }
                }
            }
...
// MARK: - OpenMenuBarItemView

private struct OpenMenuBarItemView: View {
    @EnvironmentObject var imageCache: MenuBarItemImageCache
    @EnvironmentObject var itemManager: MenuBarItemManager
    ...
    var body: some View {
        if let image {
            Image(nsImage: image)
                ...
                .overlay {
                    OpenMenuBarItemClickView(item: item, leftClickAction: leftClickAction, rightClickAction: rightClickAction)
                }
...
// MARK: - OpenMenuBarItemClickView

private struct OpenMenuBarItemClickView: NSViewRepresentable {
