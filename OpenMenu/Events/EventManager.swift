...
extension EventManager {
    ...
    private func handleSmartRehide(with event: NSEvent) {
        ...
        // Make sure clicking the OpenMenu Bar doesn't trigger rehide.
        guard event.window !== appState.menuBarManager.openMenuBarPanel else {
            return
        }
    ...
    private func handlePreventShowOnHover(with event: NSEvent) {
        guard
            ...
            !appState.settingsManager.generalSettingsManager.useOpenMenuBar,
        ...
        if isMouseInsideMenuBarItem {
            switch event.type {
            case .leftMouseDown:
                if appState.menuBarManager.sections.contains(where: { !$0.isHidden }) || isMouseInsideOpenMenuIcon {
                    // We have a left click that is inside the menu bar while at least one
                    // section is visible or the mouse is inside the OpenMenu icon.
...
extension EventManager {
    ...
    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the OpenMenu Bar panel.
    var isMouseInsideOpenMenuBar: Bool {
        guard
            let appState,
            let mouseLocation = MouseCursor.locationAppKit
        else {
        ...
        let panel = appState.menuBarManager.openMenuBarPanel
        // Pad the frame to be more forgiving if the user accidentally
        // moves their mouse outside of the OpenMenu Bar.
        let paddedFrame = panel.frame.insetBy(dx: -10, dy: -10)
        return paddedFrame.contains(mouseLocation)
    }

    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the OpenMenu icon.
    var isMouseInsideOpenMenuIcon: Bool {
        guard
            let appState,
            let visibleSection = appState.menuBarManager.section(withName: .visible),
            let openMenuIconFrame = visibleSection.controlItem.windowFrame,
            let mouseLocation = MouseCursor.locationAppKit
        else {
            return false
        }
        return openMenuIconFrame.contains(mouseLocation)
    }
}
